...
import { excelAppleIdApi } from '@/app/podrss/_lib/features/api/excelAppleIdApi';
...
export const ExcelAppleIdPage = () => {
  ...
  const handleSubmitJson = async () => {
    if (!form.file) {
      return toast.error('파일을 업로드해주세요.', {
        id: TOAST_IDS.excelAppleId.warning,
      });
    }
    ...
    try {
      ...
      toast.success('분석이 완료되었습니다.', {
        id: TOAST_IDS.excelAppleId.success,
      });
    } catch (error) {
      toast.error(handleApiError(error), { id: TOAST_IDS.excelAppleId.error });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className='space-y-6'>
      <SectionCard
        title='엑셀 Apple ID 조회'
        subtitle='Apple ID 기준 엑셀 파일을 업로드해 채널 메타데이터와 RSS 값을 공통 콘솔 안에서 조회합니다.'
      >
